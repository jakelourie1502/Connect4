...
class AZ_node():
    def __init__(self, board, action, player, model, actions=7, parent=None, done=False, cupt=1, gamma =0.9, valid=True, agent_rand=0.05):
        ...
        self.action = action
        self.model = model
        self.prob_vector, self.exp_value = self.model((torch.from_numpy(two_channel_matrix(board*self.player).reshape(1,2,6,7))).float())
        self.prob_vector = self.prob_vector.detach().numpy().flatten()
        self.agent_rand = agent_rand
        self.q_value = 0
        self.cupt = cupt
        self.actions = 7
        self.gamma = gamma
...
class Alpha_player():
    def __init__(self,model,simulations, actions=7,gamma=0.9, cupt =1,agent_rand=0.05):
        self.simulations = simulations
        self.actions = actions
        self.gamma = gamma
        self.cupt = cupt
    ...
    def make_choice(self,board,actions_,player):
        ...
        root = AZ_node(board, model = self.model, player=player,action=None, cupt=self.cupt,agent_rand=self.agent_rand)

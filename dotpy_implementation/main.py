...
def training_loop(model,base_model, training_epochs, games_per_epoch, mcts_search, batch_size, crit1, crit2, mini_epochs, cupt, rand_factor=0.2):
    ...
    for i in range(training_epochs):
        ...
        for game in range(games_per_epoch):
            ...
            states, prob_vecs, rewards= run_episode(mcts_search, model, cupt, states, prob_vecs, rewards, rand_factor=rand_factor)
        ...
        candidate_mod = train_system(dLoader, model, optimizer, crit1, crit2, mini_epochs)

    candidate_agent = Alpha_player(simulations = mcts_searches, model = model, cupt=1)
...
for loop in range(loops):
    model = training_loop(model, model_base, training_epochs, games_per_epoch, mcts_searches, batch_size, crit_1, crit_2, mini_epochs, cupt, rand_factor=0.25-loop*0.001)
